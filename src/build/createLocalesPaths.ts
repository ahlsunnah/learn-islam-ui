...
const createLocalesPaths = (
  ...
  if (!locales || !locales.length) {
    throw new Error('this function should receive an array of string with at least one element')
  }
  const localePaths = locales.reduce((acc: PartialLocalePaths, locale): PartialLocalePaths => {
    if (locale === 'ar') {
      acc[locale] = '/'
    } else {
      acc[locale] = `/${locale}/`
  ...
  const otherLocalesPaths =
    locales.length < 2
      ...
      : (locales
          .slice()
          .reverse()
          .reduce((prev: PartialLocalePaths, curr, currIndex): PartialLocalePaths => {
